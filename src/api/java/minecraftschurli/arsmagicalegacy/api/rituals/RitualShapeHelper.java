...
public final class RitualShapeHelper {
	...
	public static List<ItemStack> checkForRitual(AbstractRitual ritual, World world, BlockPos pos){
		int radius = ritual.getReagentSearchRadius();
		return getItemsInRange(world, pos, radius).stream().sorted(Comparator.comparingInt(value -> value.ticksExisted)).map(ItemEntity::getItem).collect(Collectors.toList());
	}
...
//		for (MultiblockGroup group : ritual.getRitualShape().getMatchingGroups(world, pos)) for (BlockPos blockPos : group.getPositions()) {
