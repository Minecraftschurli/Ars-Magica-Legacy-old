...
public class ResearchCapability implements ICapabilitySerializable<INBT> {
    private final LazyOptional<IResearchStorage> instance = LazyOptional.of(CapabilityHelper.getResearchCapability()::getDefaultInstance);
    ...
    public static void register() {
        CapabilityManager.INSTANCE.register(IResearchStorage.class, new Capability.IStorage<IResearchStorage>() {
            ...
            public INBT writeNBT(Capability<IResearchStorage> capability, IResearchStorage instance, Direction side) {
                ...
                instance.getLearned().stream().map(ResourceLocation::toString).map(StringNBT::valueOf).forEach(learned::add);
            ...
            public void readNBT(Capability<IResearchStorage> capability, IResearchStorage instance, Direction side, INBT nbt) {
                ...
                ((CompoundNBT) nbt).getList("learned", Constants.NBT.TAG_STRING).stream().map(INBT::getString).map(ResourceLocation::new).forEach(instance::learn);
            }
        }, ResearchStorage::new);
    }
    ...
    public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
        return cap == CapabilityHelper.getResearchCapability() ? instance.cast() : LazyOptional.empty();
    }

    @Override
    public INBT serializeNBT() {
        return CapabilityHelper.getResearchCapability().getStorage().writeNBT(CapabilityHelper.getResearchCapability(), this.instance.orElseThrow(() -> new IllegalArgumentException("LazyOptional must not be empty!")), null);
    }

    @Override
    public void deserializeNBT(INBT nbt) {
        CapabilityHelper.getResearchCapability().getStorage().readNBT(CapabilityHelper.getResearchCapability(), this.instance.orElseThrow(() -> new IllegalArgumentException("LazyOptional must not be empty!")), null, nbt);
    }
}
