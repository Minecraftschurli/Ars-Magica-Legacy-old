...
//            }
            if (!world.isRemote)
                ((LivingEntity) target).addPotionEffect(new EffectInstance(ModEffects.GRAVITY_WELL.get(), duration, 0));
            return true;
        }
        return false;
    }
...
//            }
    ...
    public ISpellIngredient[] getRecipe() {
        return new ISpellIngredient[]{
                ...
                new ItemStackSpellIngredient(new ItemStack(Items.STONE))
        };
    }

    @Override
    public float getAffinityShift(Affinity affinity) {
        return affinity == ModSpellParts.EARTH.get() ? 0.03f : 0.01f;
    }
