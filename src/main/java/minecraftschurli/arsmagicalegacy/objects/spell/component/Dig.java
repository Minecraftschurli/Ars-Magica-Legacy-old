...
public class Dig extends SpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, BlockPos blockPos, Direction blockFace, double impactX, double impactY, double impactZ, LivingEntity caster) {
        ...
        if (SpellUtils.modifierIsPresent(SpellModifiers.SILKTOUCH_LEVEL, stack)) if (EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, stack) <= 0) stack.addEnchantment(Enchantments.SILK_TOUCH, 1);
        else if (SpellUtils.modifierIsPresent(SpellModifiers.FORTUNE_LEVEL, stack) && EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, stack) <= 0) stack.addEnchantment(Enchantments.FORTUNE, SpellUtils.countModifiers(SpellModifiers.FORTUNE_LEVEL, stack));
