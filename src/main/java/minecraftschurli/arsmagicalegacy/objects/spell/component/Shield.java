...
public class Shield extends SpellComponent {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, LivingEntity caster, Entity target) {
        if (target instanceof LivingEntity) {
            if (((LivingEntity) target).isPotionActive(PotionEffectsDefs.spellReflect)) {
                return true;
            }
            ...
            if (!world.isRemote)
                ((LivingEntity) target).addPotionEffect(new BuffEffectMagicShield(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
            return true;
        }
        return false;
    }
