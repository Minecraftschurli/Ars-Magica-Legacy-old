...
//        if (!world.isRemote) world.setBlockState(pos, ModBlocks.blockMageLight.getDefaultState().withProperty(BlockMageLight.COLOR, EnumDyeColor.byMetadata(getColor(stack))));
    ...
    private int getColor(ItemStack spell) {
        ...
        if (SpellUtils.modifierIsPresent(SpellModifiers.COLOR, spell)) {
            ...
            for (SpellModifier mod : modifiers) if (mod instanceof Color) dye_color_num = spell.getTag().getInt("Color");
        }
...
//            if (!world.isRemote) ((LivingEntity) target).addPotionEffect(new IlluminationEffect(duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
            return true;
        }
        return false;
    }
