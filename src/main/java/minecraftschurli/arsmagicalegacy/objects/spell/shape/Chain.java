...
public class Chain extends SpellShape {
    ...
    public SpellCastResult beginStackStage(SpellItem item, ItemStack stack, LivingEntity caster, LivingEntity target, World world, double x, double y, double z, Direction side, boolean giveXP, int useCount) {
        ...
        for (LivingEntity e : targets) {
            ...
            SpellUtils.applyStackStage(stack, caster, e, e.getPositionVec().x, e.getPositionVec().y, e.getPositionVec().z, null, world, true, giveXP, 0);
            if (world.isRemote) {
//                if (prevEntity == null) spawnChainParticles(world, x, y, z, e.getPositionVec().x, e.getPositionVec().y + e.getEyeHeight(), e.getPositionVec().z, stack);
//                else spawnChainParticles(world, prevEntity.getPositionVec().x, prevEntity.getPositionVec().y + e.getEyeHeight(), prevEntity.getPositionVec().z, e.getPositionVec().x, e.getPositionVec().y + e.getEyeHeight(), e.getPositionVec().z, stack);
            }
            prevEntity = e;
