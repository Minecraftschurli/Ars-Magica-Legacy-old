...
public final class ParticleManager {
    ...
    public static void spawnBeam(World world, Entity source, double endX, double endY, double endZ, int color) {
        if (!(source instanceof PlayerEntity) || source == ArsMagicaLegacy.proxy.getLocalPlayer()) spawnBeam(world, source.posX, source.posY, source.posZ, endX, endY, endZ, color);
        else spawnBeam(world, source.posX, source.posY + source.getEyeHeight() - 0.2f, source.posZ, endX, endY, endZ, color);
    }
