...
public class OcculusScreen extends Screen {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) { // fixme
        ...
        if (currentTree != ModSkillTrees.AFFINITY.get()) {
            renderSkillTreePage(mouseX, mouseY, posX, posY, renderSize, renderRatio, calcYOffest, calcXOffest);
        } else {
            renderAffinityPage(mouseX, mouseY, posX, posY);
        }
        RenderSystem.color3f(1, 1, 1);
        super.render(mouseX, mouseY, partialTicks);
    }

    private void renderSkillTreePage(int mouseX, int mouseY, int posX, int posY, float renderSize, float renderRatio, float calcYOffest, float calcXOffest) {
    ...
    private void renderAffinityPage(int mouseX, int mouseY, int posX, int posY) {
        ...
        for (Affinity aff : RegistryHandler.getAffinityRegistry().getValues()) {
            if (Objects.equals(aff.getRegistryName(), Affinity.NONE))
                continue;
            double depth = CapabilityHelper.getAffinityDepth(player, aff) / 100;
            ...
            getMinecraft().getItemRenderer().renderItemAndEffectIntoGUI(IAffinityItem.getStackForAffinity(ModItems.AFFINITY_ESSENCE.get(), aff), drawX, drawY);
