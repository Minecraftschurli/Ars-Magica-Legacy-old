...
public class BlackAuremBlock extends EtheriumGeneratorBlock<BlackAuremTileEntity> {
    ...
    public int getTier(BlockState state, World world, BlockPos pos) {
        ...
        if (BLACK_AUREM_CHALK.get().validate(world, pos) != null) {
            if (BLACK_AUREM_PILLAR_1.get().validate(world, pos) != null) tier = 2;
            else if (BLACK_AUREM_PILLAR_2.get().validate(world, pos) != null) tier = 3;
            else if (BLACK_AUREM_PILLAR_3.get().validate(world, pos) != null) tier = 4;
            else tier = 1;
        }
        return tier;
    }
