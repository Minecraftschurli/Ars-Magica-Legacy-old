...
public final class LavaWalkerAbility extends AbstractToggledAffinityAbility {
    @Override
        ...
        if (!player.world.isRemote) {
            ...
            for (int x = -1; x <= 1; x++)
                for (int z = -1; z <= 1; z++) {
                    ...
                    if (player.world.getFluidState(newPos).getFluid() == Fluids.LAVA || player.world.getFluidState(newPos).getFluid() == Fluids.FLOWING_LAVA)
                        player.world.setBlockState(newPos, Blocks.OBSIDIAN.getDefaultState());
                }
        }
    }
