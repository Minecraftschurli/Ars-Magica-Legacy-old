        ...
        ForgeRegistries.BIOMES.getValues()
                .forEach(biome -> {
                    ...
                    if (BiomeFilter.OVERWORLD.apply(biome)) {
                        if(Config.SERVER.METEOR_FREQUENCY.get() > 0) biome.addFeature(GenerationStage.Decoration.SURFACE_STRUCTURES, ModFeatures.MOONSTONE_METEOR.get().withConfiguration(NoFeatureConfig.NO_FEATURE_CONFIG).withPlacement(Placement.COUNT_HEIGHTMAP.configure(new FrequencyConfig(Config.SERVER.METEOR_FREQUENCY.get()))));
