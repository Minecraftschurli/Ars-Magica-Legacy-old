	...
	private void dropHead(Entity target, World world){
		if (target.getClass() == EntitySkeleton.class){
			if (((EntitySkeleton)target).getSkeletonType() == 1){
				doDropHead(world, target.posX, target.posY, target.posZ, 1);
			}else{
				doDropHead(world, target.posX, target.posY, target.posZ, 0);
			}
		}else if (target.getClass() == EntityZombie.class){
			doDropHead(world, target.posX, target.posY, target.posZ, 2);
		}else if (target.getClass() == EntityCreeper.class){
			doDropHead(world, target.posX, target.posY, target.posZ, 4);
		}else if (target instanceof EntityPlayer){
			doDropHead(world, target.posX, target.posY, target.posZ, 3);
		}
	}

	private void doDropHead(World world, double x, double y, double z, int type){
